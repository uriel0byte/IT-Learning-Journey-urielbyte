...
**Significance:** This is **enumeration**—attacker is actively testing for exploitable vulnerabilities, not just passively gathering info.
...
**Attack progression:**
1. SQL injection → gain database access
...
- Starting broad (all logs) → narrowing down (specific IP)
...
- Recon → Exploitation → Exfiltration → Ransomware
...
Alert fires for unusual traffic → Query Splunk → Filter benign traffic → Identify suspicious IP → Trace activity → Escalate to incident response

**Example 2 - Threat Hunting:**
Hypothesis: "Are there any SQL injection attempts in our environment?" → Write SPL query for SQL injection indicators → Find attempts → Investigate source IPs → Check if exploitation successful

**Example 3 - Incident Response:**
Server compromised → Search all logs for that server IP → Identify initial access vector → Trace lateral movement → Calculate data exfiltration → Build incident timeline

**Career Skills Developed:**
- ✅ **SIEM proficiency** - Splunk (industry-standard)
- ✅ **SPL mastery** - Writing effective queries
- ✅ **Log analysis** - Finding needles in haystacks
- ✅ **Threat detection** - Identifying attack patterns
- ✅ **Incident investigation** - Reconstructing attack timelines
- ✅ **Data correlation** - Connecting evidence across sources
- ✅ **Critical thinking** - Understanding attacker mindset
- ✅ **Technical documentation** - Recording findings clearly

**Interview Talking Point:** "I have hands-on experience conducting SIEM investigations using Splunk, where I analyzed web traffic and firewall logs to reconstruct a complete ransomware attack chain—from initial reconnaissance through SQL injection exploitation, webshell deployment, RCE, C2 communication, and data exfiltration. I can write complex SPL queries using boolean operators, aggregation functions, and wildcards to hunt for threats. I understand how to correlate logs across multiple sources, identify attack patterns in large datasets, filter benign traffic to focus on true threats, and reconstruct attack timelines chronologically. I've practiced the complete SOC analyst workflow: alert triage, threat hunting, incident investigation, and quantifying breach impact. This directly applies to daily SOC operations monitoring security alerts and investigating incidents in production environments."
...
# span=1h   → Group by 1 hour
# span=15m  → Group by 15 minutes
# count     → Count events per time bucket
```
...
**⚠️ CRITICAL:** Field names differ between sources!
...
**⚠️ Context matters:** `curl` and `wget` aren't always malicious, but non-browser user agents deserve investigation.
...
→ Pivot to firewall logs to see where that IP connected

**Scenario 2: Found compromised server in firewall logs**
→ Pivot to web logs to see how it was compromised

**Scenario 3: Found malware indicator in endpoint logs**
→ Pivot to network logs to see C2 communication
...
user_agent=*sqlmap*      # ✅ Matches "sqlmap/1.0", "sqlmap-dev"
```
